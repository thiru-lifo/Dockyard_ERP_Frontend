...
export class RankComponent implements OnInit {
  ...
  getRank() {
    this.api
      ...
      .subscribe((res) => {
        ...
        this.logger.log('rankkkkkk',this.rank)
      });
  }
  paygrades:any
  getPayGrade(){
    this.api.getAPI(environment.API_URL + "master/pay_grade")
      .subscribe((res) => {
        this.paygrades= res.data;
        // this.dataSource.paginator = this.pagination;
        this.logger.log('paygradddd',this.paygrades)
      });
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/rank/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/rank/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
