...
@Component({
  ...
  templateUrl: './over_time.component.html',
  styleUrls: ['./over_time.component.scss']
})
...
over_times:any
  getOverTime() {
    this.api
      .getAPI(environment.API_URL + "master/over-time")
      .subscribe((res) => {
        this.dataSource = new MatTableDataSource(res.data);
        this.over_times = res.data;
        this.dataSource.paginator = this.pagination;
        this.logger.log('over_times',this.over_times)
      });
  }
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/over-time/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Over time '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getOverTime();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/over-time/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
