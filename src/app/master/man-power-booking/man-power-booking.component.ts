...
@Component({
  ...
  templateUrl: './man-power-booking.component.html',
  styleUrls: ['./man-power-booking.component.scss']
})
export class ManPowerBookingComponent implements OnInit {
  ...
  getSubModule() {
    if(this.param==undefined) this.param=""; else this.param;
    this.api
      .getAPI(environment.API_URL + "master/sub_module/man_power_booking?"+this.param)
      .subscribe((res) => {
        this.dataSource = new MatTableDataSource(res.data);
        this.countryList = res.data;
        this.dataSource.paginator = this.pagination;
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/sub_module/man_power_booking/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/sub_module/man_power_booking/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
