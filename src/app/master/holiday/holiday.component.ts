...
holiday:any
  getHoliday() {
    this.api
      .getAPI(environment.API_URL + "master/holiday")
      .subscribe((res) => {
        this.dataSource = new MatTableDataSource(res.data);
        this.holiday = res.data;
        this.dataSource.paginator = this.pagination;
        this.logger.log('holiday',this.holiday)
      });
  }
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/holiday/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          ...
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Holiday '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getHoliday();
          } else {
  ...
  onSubmit() {
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/holiday/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
