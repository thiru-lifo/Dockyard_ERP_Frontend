...
export class BatchComponent implements OnInit {
  ...
  populate(data) {
    ...
    //  this.api
    //  .getAPI(environment.API_URL + "master/get_batch?id="+data.id)
    // .subscribe((res) => {
      //this.api.displayLoading(false)

      // let batchname_data = res.data.batchname;

      //console.log(data,"Form level hierarachy")
      //this.showErrors=false;
      // this.clearHierarchyFormArray(this.batchname);

      // //setTimeout(()=>{
        // if(batchname_data.length>0)
        // {
        //   for(let i=0;i<batchname_data.length;i++)
        //   {
        //     this.addBatchname(batchname_data[i])                  
        //   }
        // }
      // //},2000)
...
batchs:any
  getBatch() {
    ...
    this.api
      .getAPI(environment.API_URL + "master/batch")
      .subscribe((res) => {
        this.dataSource = new MatTableDataSource(res.data);
        this.batchs = res.data;
        this.dataSource.paginator = this.pagination;
        this.logger.log('batchs',this.batchs)
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.api.postAPI(environment.API_URL + "master/batch/details", {
          id: id,
          status: 3,
        }).subscribe((res)=>{
          this.logger.log('response',res);
          if(res.status==environment.SUCCESS_CODE) {
            ...
            this.notification.warn('Batch '+language[environment.DEFAULT_LANG].deleteMsg);
            this.getBatch();
          } else {
            this.notification.displayMessage(language[environment.DEFAULT_LANG].unableDelete);
          }
  ...
  onSubmit() {
    ...
    //return false;
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "master/batch/details",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
...
//   get batchname() : FormArray {
//     return this.editForm.get("batchname") as FormArray
//   }

//   addBatchname(data='') {
//     console.log("aaaaa")
//      this.batchname.push(this.newBatchname(data));
//   }
//  new:any
//   newBatchname(data): FormGroup {
...
//  removeBatchname(i:number){
//     this.batchname.removeAt(i);
//   }
