...
export class WorkInstructionComponent implements OnInit {
  ...
  public editForm = new FormGroup({
    ...
    dart: new FormControl("",[Validators.required]),
    man_days: new FormControl(""),
    start_date: new FormControl("moment().format('MM/DD/yyyy')"),
    end_date: new FormControl("moment().format('MM/DD/yyyy')"),
    status_description: new FormControl(""),
    WI_close_date: new FormControl("moment().format('MM/DD/yyyy')"),
  ...
  getListing() {
    this.api
      .getAPI(environment.API_URL + "transaction/wi")
      .subscribe((res) => {
  ...
  onSubmit() {
    ...
    this.showError = true;
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/wi/crud",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
  ...
  getDart() {
    this.api
      .getAPI(environment.API_URL + "transaction/dart?status=1")
      .subscribe((res) => {
        this.darts = res.data;
        console.log(this.darts,"darts")
      });
  }
  ...
  getRefit() {
    this.api
      ...
      .subscribe((res) => {
        this.refits = res.data;
        console.log(this.refits,"refit")
      });
  }
...
numberOnly(event:any): boolean {
  ...
  onSubmitQC() {
    ...
    if (this.editFormQC.valid) {
      this.api
      .postAPI(
        environment.API_URL + "transaction/wi/qc/crud",
  ...
  openQCStatus(country){
    ...
    this.api.postAPI(environment.API_URL + "transaction/wi/qc", {
      work_instruction_id: country,
    }).subscribe((res) => {
