...
export class GeneratePaySlipComponent implements OnInit {
  ...
  public editForm = new FormGroup({
    ...
    for_month: new FormControl("", [Validators.required]),
  });
  ...
  onSubmit() {
    ...
    this.showError = true;
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/generate-pay-slip",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);

          window.open(environment.API_URL+'transaction/generate-pay-slip-pdf/'+this.editForm.value.user)
          //alert(res.status)
