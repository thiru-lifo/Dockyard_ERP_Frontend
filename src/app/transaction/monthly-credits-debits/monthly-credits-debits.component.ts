...
@Component({
  selector: 'app-monthly-credits-debits',
  templateUrl: './monthly-credits-debits.component.html',
  styleUrls: ['./monthly-credits-debits.component.scss']
})
export class MonthlyCreditsDebitsComponent implements OnInit {
  ...
  public editForm = new FormGroup({
    ...
    for_month: new FormControl("moment().format('MM/DD/yyyy')"),
    allowance: new FormControl("",[Validators.required]),
    allowance_amount: new FormControl("",[Validators.required]),
    deduction: new FormControl("",[Validators.required]),
    deduction_amount: new FormControl("",[Validators.required]),
  ...
  getListing() {
    this.api
      .getAPI(environment.API_URL + "transaction/monthly-credits-debits")
      .subscribe((res) => {
  ...
  onSubmit() {
    ...
    this.showError = true;
     if (this.editForm.valid) {
      ...
      this.api
        .postAPI(
          environment.API_URL + "transaction/monthly-credits-debits/crud",
          this.editForm.value
        )
        .subscribe((res) => {
          this.logger.log('response',res);
...
employees:any;
  ...
  getAllowances() {
    this.api
      .getAPI(environment.API_URL + "master/allowances_master?status=1")
      .subscribe((res) => {
  ...
  getDeductions() {
    this.api
      .getAPI(environment.API_URL + "master/deductions_master?status=1")
      .subscribe((res) => {
